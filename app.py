...
def get_product_by_id(product_id):
    ...
    for p in products:
        if p["id"] == product_id:
            product = p
            break
    return (jsonify(product), 200) if product else ('product not found', 404)
...
def create_products():
    ...
    return jsonify(new_product), 201  # Return the new product in JSON format with status code 201
